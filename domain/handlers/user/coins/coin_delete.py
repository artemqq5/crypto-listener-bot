...
async def coin_delete_confirm_call(
    ...
    if not await CoinsRepository().delete(data["coin"]["coinname"]):
        await callback.message.edit_text(
            i18n.COIN.DELETE.FAIL(coin=data["coin"]["coin_label"]),
            reply_markup=kb_back_coins_nav,
        )
        return

    await callback.message.edit_text(
        i18n.COIN.DELETE.SUCCESS(coin=data["coin"]["coin_label"]),
        reply_markup=kb_back_coins_nav,
    )
